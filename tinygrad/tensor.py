...
class Tensor(MathTrait):
  ...
  def _pool(self, k_:tuple[sint, ...], stride:int|tuple[int, ...]=1, dilation:int|tuple[int, ...]=1) -> Tensor:
    # NOTE: The following would match torch behaviour
    # assert any(resolve(k > 0) for k in k_), f"kernel size must be greater than 0, got {k_}"
    ...
    # input size scaling factor to make sure shrink for stride is possible (i * f + d >= o * s)
    f_ = [ceildiv(o*s - d, i) if resolve(o*s - d > 0, default=False) else 1 for o,s,i,d in zip(o_, s_, i_, d_)]
