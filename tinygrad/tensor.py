  ...
  def rand(*shape, device:str|None=None, dtype:DTypeLike|None=None, contiguous:bool=True, **kwargs) -> Tensor:
    ...
    if device not in Tensor._device_seeds:
      Tensor._device_seeds[device] = Tensor(
        (Tensor._seed << 32) | (int.from_bytes(hashlib.sha256(len(Tensor._device_seeds).to_bytes(4, "big")).digest(), "big") & 0xffffffff),
    ...
    counts0 = counts1 = (Tensor.arange(ceildiv(num, 2), device=device, dtype=dtypes.uint32, requires_grad=False)+bits_count)
